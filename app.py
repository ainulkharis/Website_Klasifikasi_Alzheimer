...
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///alzha.db'
db = SQLAlchemy(app)
...
app.config['MAIL_USERNAME'] = 'alzhateam@gmail.com'
app.config['MAIL_DEFAULT_SENDER'] = 'alzhateam@gmail.com'
app.config['MAIL_PASSWORD'] = 'yxqf jtdz alwj ljhj'
...
def dashboard():
    if request.method == 'POST':
        ...
        session['success_message'] = 'Komentar berhasil terkirim'
        ...
        if request.referrer and "/dashboard" in request.referrer:
            ...
            return render_template('dashboard.html', chart_path=chart_path, total_count=total_count, comments=comments, positive_count=positive_count, negative_count=negative_count, netral_count=netral_count, klasifikasi_class=existing_klasifikasi.klasifikasi_class if existing_klasifikasi else None, probabiliti=existing_klasifikasi.probabiliti if existing_klasifikasi else None, image=existing_klasifikasi.image_path if existing_klasifikasi else None, nm=existing_klasifikasi.nama if existing_klasifikasi else None, tgl=existing_klasifikasi.tanggal if existing_klasifikasi else None, email=existing_klasifikasi.email if existing_klasifikasi else None, umr=existing_klasifikasi.umur if existing_klasifikasi else None, jk=existing_klasifikasi.jenis_kelamin if existing_klasifikasi else None, riwayat_klasifikasi=riwayat_klasifikasi)
